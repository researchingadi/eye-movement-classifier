...
**Status:** Active — Encoding Phase complete
...
### `step4c_permutation_test.py`

**Purpose:** Final statistical validation. Tests whether the observed
LOSO AUC is above chance by comparing it against 1000 null AUCs
generated by shuffling task labels and rerunning LOSO. Produces the
permutation p-value needed to complete the publication result statement.

**Method (updated):**
Labels shuffled WITHIN each subject (not globally) to preserve
subject-level trial counts, class balance, and LOSO fold structure.
p-value uses plus-one correction: p = (n_exceed + 1) / (n + 1).
Minimum possible p = 0.001 with 1000 permutations.
scikit-learn RF uses CPU only — no GPU acceleration.

**Outputs:**
- permutation_results.json — p-values and null distribution statistics
...
**Final parameters (confirmed):**
- N_PERMUTATIONS = 200 (minimum p = 1/201 = 0.005)
- N_TREES_NULL = 100
- Within-subject label shuffling
- Plus-one p-value correction
- RF only
- Estimated runtime: ~9-10 hours on Colab CPU
- Note: a partial run of 200 permutations from an earlier 500-permutation
  attempt confirmed null AUCs ranging 0.473-0.528, zero exceeding
...
### Whitlock query — June 2026 (foils at retrieval)
Whitlock noted that foils play no role in the encoding classifier
(target==1 filter) but will be present and meaningful at test.
Key distinction: Item task foils are novel/unstudied objects;
Relational task foils are studied objects paired with different scenes.
This means Relational test requires active discrimination of the
associate from familiar foils using scene context — predicted to
produce more comparative scanning and scene-guided looking.
Foil-related features (dwell on foils, inter-object transitions,
