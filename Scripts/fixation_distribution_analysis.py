...
#
# PURPOSE:
#   Plot the distribution of fixation counts per trial, separately for
#   Item and Relational tasks. This plot is the direct input to the
#   decision about minimum fixation threshold for trial exclusion.
#
...
print(f"Trial-level data: {len(trial_fixation_counts):,} trials total")
print()
...
# ─────────────────────────────────────────────────────────────────────────────
item_1fix = (item_counts == 1).sum()
rel_1fix  = (rel_counts  == 1).sum()
item_3fix = (item_counts <= 3).sum()
rel_3fix  = (rel_counts  <= 3).sum()

annotation = (
    f"Trials with exactly 1 fixation:\n"
    f"  Item: {item_1fix} ({100*item_1fix/len(item_counts):.1f}%)\n"
    f"  Relational: {rel_1fix} ({100*rel_1fix/len(rel_counts):.1f}%)\n\n"
    f"Trials with ≤ 3 fixations:\n"
    f"  Item: {item_3fix} ({100*item_3fix/len(item_counts):.1f}%)\n"
    f"  Relational: {rel_3fix} ({100*rel_3fix/len(rel_counts):.1f}%)"
)
...
OUTPUT_PATH = "fixation_distribution.png"
